...
Climbing down the ladder, you discover the source of the <span title="The beeping is unrelated to the issue with the server rack. The reactor is just hungry.">beeping</span>: a large, toroidal reactor which powers the factory above. Some Elves here are hurriedly running between the reactor and a nearby server rack, apparently trying to fix something.
